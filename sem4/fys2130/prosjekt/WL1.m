...
% Morlet waveletanalysen skal gjennomf�res i frekvensintervallet
% [fmin,fmax] med K-parameteren K. Se detaljer i et kapittel i l�reboka
% Svingninger og b�lger, Arnt Inge Vistnes, 2014.
...
% Beregner # analysefrekvenser, skriver til skjerm, klargj�r frekvensene
...
%L�kke over alle frekvenser som inng�r i analysen
for jj=1:M
    ...
    %Beregners�enhellinjeiwaveletdiagrammeti�njafs!
    %WLdiagram(jj,:)=abs(ifft(FTwl.*transpose(FTsignal))); %Ettalternativ
    %WLdiagram(jj,:)=sqrt(abs(ifft(FTwl.*(FTsignal))));%Ettannet
    WLdiagram(jj,:)=sqrt(abs(ifft(FTwl.*transpose(FTsignal))));%Ett til
    
    %Brukerdensistevariantenfor�f�svakepartierbedresynlig
    fbrukt(jj)=f_analyse; %Lagrerfrekvensenesomfaktiskerbrukt
    f_analyse=f_analyse*ftrinn; %Beregnernestefrekvens
end;
%Redusererfilst�rrelseved�fjernemyeavoverfl�diginformasjonitid.
%Dettegj�reskunforatfilst�rrelsenp�plotteneskalblih�ndterbar.
...
%Foretaenmarkeringiplottetfor�viseomr�dermedrandproblemer
...
%title('WaveletPowerSpektrum');%Velgdennen�rdeteraktuelt,
title('Sqrt(WaveletPowerSpektrum)');%mendennen�rsqrtblirbrukt
