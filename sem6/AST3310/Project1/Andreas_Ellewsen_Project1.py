...
def Opacity(T,rho):
	...
	log10kappa = lines[Ti][Ri] #Reads the value in the right column
	kappa = (10**(log10kappa/10))	#Convert the value given to the form used in the equations
	return kappa
...
N 	 = 1E7		#Number of iterations. Should be large, at least 10**4, preferably 10**6.
...
while M[i] > 0 and R[i] > 0 and L[i] > 0 and T[i] > 0 and P[i] > 0:
    ...
    drdm    = 0#1./(4*pi*R[i]**2*rho[i])	#Change in radius
    dm1     = p*R[i]/drdm

    dPdm    = -(G*M[i])/(4*pi*R[i]**4)	#Change in Pressure
    dm2     = p*P[i]/dPdm

    dLdm    = 0#epsilon[i]		#Change in Luminosty
    dm3     = p*L[i]/dLdm
    
    kappa   = Opacity(T[i],rho[i])	#Opacity
    dTdm    = -3*kappa*L[i]/(256*pi**2*sigma*R[i]**4*T[i]**3)	#Change in temperature
    dm4     = p*T[i]/dTdm

    dm5 = p*M[i]
    dm  = -min(abs(dm1),abs(dm2),abs(dm3),abs(dm4),abs(dm5))
    
    #rho[i+1]  = 1/(4*pi*R[i]**2*dr)*dm	#Update density with geometry
    rho[i+1]  = Density(P[i],T[i])	#Update Density with ideal gas law
    M[i+1]    = M[i] + dm		#Update mass
    R[i+1]    = R[i] + drdm*dm		#Update Radius
    L[i+1]    = L[i] + dLdm*dm 		#Update Luminosity
    P[i+1]    = P[i] + dPdm*dm		#Update Pressure
    T[i+1]    = T[i] + dTdm*dm		#Update Temperature
    epsilon[i+1] = Energy(T[i],rho[i])    #Update energy output
    
    print 'M = %.3e, R = %.3e ,L = %.3e ,P = %.3e , T= %.3e, rho = %.3e, \
kappa = %.3e, epsilon = %.3e, dT = %.3e, dr = %.3e, dL = %.3e, dm = %.3e, \
i = %.3e'%(M[i], R[i], L[i], P[i], T[i], rho[i], kappa, epsilon[i], \
    dTdm*dm, drdm*dm, dLdm*dm, dm,i)
    ...
    if abs(dm) < 1E1:
        print 'dm went too small'        
        sys.exit()

#Remove the last entry since they are unphysical
M[-1] = 0
R[-1] = 0
L[-1] = 0
P[-1] = 0
...
#Remove all the unused values at the end of the arrays
trim_zeros(M)
trim_zeros(R)
trim_zeros(L)
trim_zeros(P)
