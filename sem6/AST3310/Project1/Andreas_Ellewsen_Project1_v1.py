...
chain are calculated. We neglect the other processes since we choose to 
...
def reactions(T):
	...
	He3_Be    = 	5.61E6*T9_star1**(5./6)*T9**(-3./2)*exp(-12.826*T9_star1\
                     **(-1./3))
...
dm 	 = -1E24		#Fairly large dm but still a very small fraction of the total Mass.
...
for i in range(0,N-1):
    ...
    print 'M = %.3e, R = %.3e ,L = %.3e ,P = %.3e , T= %.3e, rho = %.3e,kappa = %.3e, epsilon = %.3e, dT = %.3e, dr = %.3e, dL = %.3e'%(M[i],R[i],L[i],P[i],T[i],rho[i],kappa,epsilon[i],dT,dr,dL)
